...
public class Parser
{
    ...
    {
        Result<IStatement, string> statement = ParseFunctionDeclarationStatement();
    ...
    private Result<IStatement, string> ParseFunctionDeclarationStatement()
    {
        Option<string> access = Option<string>.None;

        if (Current() is AccessToken)
        {
            if (!NextIs<AccessToken, FunctionToken>(out var accessToken, out _, doNext: false))
                return ParseVariableDeclarationStatement();

            Next();
            access = Some(accessToken!.Symbol);
        }

        if (Current() is not FunctionToken)
            return ParseVariableDeclarationStatement();
        ...
        Option<Result<IdentifierToken, string>> returnType = None<Result<IdentifierToken, string>>();

        if (hasExplicitReturnType)
        {
            Next();
            returnType = Some(Expect<IdentifierToken>("Expected return type identifier."));
        }

        Result<IExpression, string> body = ParseBlock();

        return identifier.AndThen(
            i =>
                openParen.And(
                    parameters.AndThen(ps =>
                        closeParen.And(
                            returnType.Transpose().AndThen(rt =>
                                body.Map(b => new FunctionDeclarationStatement(
                                    access,
                                    i.Symbol,
                                    ps,
                                    rt.Map(r => r.Symbol),
    ...
    private Result<IStatement, string> ParseVariableDeclarationStatement()
    {
        ...
        return Ok<IStatement, string>(
            new VariableDeclarationStatement(typeIdentifier!.Symbol, mutable, new Identifier(Next().Symbol)));
    }
    ...
    private Result<IStatement, string> ParseIfStatement()
    {
        ...
        {
            return ifCondition.AndThen(
                ifc => ifBlock.Map(
                    ifb => new IfStatement(
                        new IfStatement.ConditionBlock(ifc, ifb),
        ...
        if (elseIfStatements.Count > 0 && elseBlock.IsNone())
        {
            return ifCondition.AndThen(
                ifc => ifBlock.AndThen(
                    ifb => elseIfStatements.Select(
                            elif =>
                            {
                                return elif.Condition.AndThen(
                                    elifc =>
        ...
        if (elseIfStatements.Count == 0 && elseBlock.IsSome())
        {
            return ifCondition.AndThen(
                ifc => ifBlock.AndThen(
                    ifb => elseBlock.Unwrap()
                        .Map(
                            eb =>
                                new IfStatement(
                                    new IfStatement.ConditionBlock(ifc, ifb),
                                    None<IEnumerable<IfStatement.ConditionBlock>>(),
                                    Some(eb)) as IStatement)));
        }
    ...
    private Result<IStatement, string> ParseStructDeclarationStatement()
    {
        ...
        Result<IdentifierToken, string> structNameIdentifier =
            Expect<IdentifierToken>("Expected identifier after 'struct' keyword.");
        ...
        return structNameIdentifier.AndThen(
            sni =>
                openCurly.AndThen(
                    _ =>
                        fields.Invert()
                            .AndThen(
                                fs =>
                                    closeCurly.Map(
                                        _ => new StructDeclarationStatement(
                                            accessModifier,
                                            sni.Symbol,
                                            fs) as IStatement))));
        ...
        Result<StructDeclarationStatement.Field, string> ParseField()
        {
            ...
            return identifier.AndThen(
                i =>
                    colon.AndThen(
                        _ =>
                            typeIdentifier.AndThen(
                                ti =>
                                    semi.Map(
                                        _ => new StructDeclarationStatement.Field(
                                            accessModifier,
                                            mutable,
                                            ti.Symbol,
                                            i.Symbol)))));
    ...
    private Result<IStatement, string> ParseUnionDeclarationStatement()
    {
        ...
        Result<IdentifierToken, string> unionNameIdentifier =
            Expect<IdentifierToken>("Expected identifier after 'union' keyword.");
        ...
        return unionNameIdentifier.AndThen(
            sni =>
                openCurly.AndThen(
                    _ =>
                        members.Invert()
                            .AndThen(
                                ms =>
                                    closeCurly.Map(
                                        _ => new UnionDeclarationStatement(
                                            accessModifier,
                                            sni.Symbol,
                                            ms) as IStatement))));
        ...
        Result<UnionDeclarationStatement.Member, string> ParseMember()
        {
            ...
            if (Current() is not OpenParenToken)
            {
                ...
                return semiColon.AndThen(
                    _ => identifier.Map(
                        ident => new UnionDeclarationStatement.Member(
                            ident.Symbol,
                            new List<UnionDeclarationStatement.Member.Field>())));
            }
                        ...
                        .AndThen(
                            fs =>
                                close.AndThen(
                                    _ =>
                                        semi.Map(
                                            _ => new UnionDeclarationStatement.Member(ident.Symbol, fs.ToList())))));
            ...
            Result<UnionDeclarationStatement.Member.Field, string> ParseField()
            {
                ...
                return fieldIdentifier.AndThen(
                    fi =>
                        colon.AndThen(
                            _ =>
                                typeIdentifier.Map(
                                    ti => new UnionDeclarationStatement.Member.Field(
                                        ti.Symbol,
                                        fi.Symbol))));
            }
        }
    ...
    private Result<IExpression, string> ParseDrop()
    {
        ...
        Result<IdentifierToken, string> identifier =
            Expect<IdentifierToken>("Expected identifier after 'drop' keyword.");
    ...
    {
        if (!NextIs<IdentifierToken, ColonToken, ColonToken, IdentifierToken>(
                out IdentifierToken? typeIdentifier,
                out _,
                out _,
                out IdentifierToken? memberIdentifier))
            return ParseAssignmentExpression();

        if (Current() is not OpenParenToken)
            return Ok<IExpression, string>(
                new UnionLiteral(
                    typeIdentifier!.Symbol,
                    memberIdentifier!.Symbol,
                    new List<UnionLiteral.FieldInitializer>()));
    ...
    private Result<IExpression, string> ParseAssignmentExpression()
    {
        ...
        return member.Map(m => (MemberExpression)m)
            .AndThen(
                m => equals.AndThen(_ => ParseExpression().Map(e => new AssignmentExpression(m, e) as IExpression)));
    }
    ...
    private Result<IExpression, string> ParseTernaryExpression()
    {
        ...
        return condition.AndThen(
            c => thenExpression.AndThen(t => elseExpression.Map(e => new TernaryExpression(c, t, e) as IExpression)));
    }
    ...
    private Result<IExpression, string> ParseComparisonExpression()
    {
        ...
        while (Current() is ComparisonOperatorToken
               {
                   Symbol: TokenSymbol.LOGICAL_EQUAL or TokenSymbol.LOGICAL_NOT_EQUAL or TokenSymbol.GREATER or
                   TokenSymbol.LESS or TokenSymbol.LOGICAL_GREATER_EQUAL or TokenSymbol.LOGICAL_LESS_EQUAL
               })
        {
            string @operator = Next().Symbol;
    ...
    private Result<IExpression, string> ParseMultiplicativeExpression()
    {
        ...
        while (Current() is ArithmeticOperatorToken
               {
                   Symbol: TokenSymbol.STAR or TokenSymbol.SLASH or TokenSymbol.PERCENT
               })
        {
            string @operator = Next().Symbol;
    ...
    private Result<IExpression, string> ParseVariableDeclarationExpression()
    {
        ...
        Result<IdentifierToken, string> identifier = typeIdentifier.AndThen(
            ti =>
    ...
    private Result<IExpression, string> ParseCallExpression(Result<IExpression, string> caller)
    {
        ...
        Result<IExpression, string> call =
            caller.AndThen(c => arguments.Map(args => new CallExpression(c, args) as IExpression));
    ...
    {
        Result<List<IExpression>, string> args = ParseExpression()
            .Map(
                e => new List<IExpression>
                {
                    e
                });
    ...
    private Result<IExpression, string> ParsePrimaryExpression()
    {
        ...
        switch (token)
        {
            ...
            case NumberToken numberToken:
                ...
                if (numberSymbol.Contains(TokenSymbol.PERIOD_CHAR) ||
                    numberSymbol.EndsWith(TypeChecker.Type.f32.ToString()))
                {
    ...
    private Result<T, string> Expect<T>(string errorMessage)
        where T : IToken
    {
        ...
        return Error<T, string>(
            $"Unexpected token found during parsing: {token}. Expected {typeof(T)} | {errorMessage}");
    }
    ...
    private bool NextIs<T1, T2, T3, T4>(
        out T1? token,
        out T2? nextToken,
        out T3? nextNextToken,
        out T4? nextNextNextToken,
        bool doNext = true)
        where T1 : class, IToken
        where T2 : class, IToken
        where T3 : class, IToken
    ...
    private bool ExistsBeforeSemiColon<T>()
        where T : IToken
    {
        int currentIndex = 0;
